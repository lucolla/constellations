...
function exportChildren(dupObj, orgObj, exportInfo, dupDocRef, fileNamePrefix) {
    for( var i = 0; i < dupObj.artLayers.length; i++) {
        ...
        if ((psdIndex == exportInfo.fileType)||(png24Index == exportInfo.fileType)||(png8Index == exportInfo.fileType) || (jpegIndex == exportInfo.fileType)) { // PSD: Keep transparency
            removeAllInvisible(duppedDocumentTmp);
  		    if ((jpegIndex == exportInfo.fileType)) { 
					if (activeDocument.activeLayer.isBackgroundLayer == false) { //is it anything but a background layer?

					    var oldWidth = app.activeDocument.width;
                        var oldHeight = app.activeDocument.height;
                        app.activeDocument.trim(TrimType.TRANSPARENT, true, true, false, false); //Trim the top & left
                        var newHeight = app.activeDocument.height;
                        var newWidth = app.activeDocument.width;

                        var xlocation = oldWidth - newWidth;
                        var ylocation = oldHeight - newHeight;



                        app.activeDocument.trim(TrimType.TRANSPARENT, false,false,true,true); //Trim the rest
        ...
        fileNameBody += "_" + xlocation.toString() + "_" + ylocation.toString();
